 ...
 * Calculates quantities from order breakup × (1 + extraPercentage/100)
...
export const getOrderLabelRequirements = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    logError('Get order label requirements error', error);
    res.status(500).json({
      error: 'Internal Server Error',
      message: 'Failed to calculate label requirements',
    });
  }
};
...
export const createLabelOverride = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    logError('Create label override error', error);
    res.status(500).json({
      error: 'Internal Server Error',
      message: 'Failed to create label override',
    });
  }
};
...
export const getOrderLabelOverrides = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    logError('Get order label overrides error', error);
    res.status(500).json({
      error: 'Internal Server Error',
      message: 'Failed to fetch label overrides',
    });
  }
};
...
export const deleteLabelOverride = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    if (!override) {
      res.status(404).json({
        error: 'Not Found',
        message: 'Label override not found',
      });
      return;
    }
    ...
    logError('Delete label override error', error);
    res.status(500).json({
      error: 'Internal Server Error',
      message: 'Failed to delete label override',
    });
  }
};
...
export const getOrderTotalLabelRequirements = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    logError('Get order total label requirements error', error);
    res.status(500).json({
      error: 'Internal Server Error',
      message: 'Failed to calculate order label requirements',
    });
  }
};
