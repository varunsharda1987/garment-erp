...
/**
 * Issue a job work order — ONE atomic transaction. Throws JobWorkOrderError (422 codes)
 * on any blocker; on success the JWO is AT_MILL/ISSUED with an ISSUED outward challan.
 */
export async function issueJobWorkOrder(jwoId: string, opts: IssueJwoOptions): Promise<IssueJwoResult> {
  ...
  const result = await prisma.$transaction(
    async (tx) => {
      // 1. MUTEX — the JWO row itself; a concurrent issue loses here and its tx rolls back.
      ...
      const itemDescription = `${jwo.processType} job work — ${jwo.jobWorkNumber}${jwo.style?.styleCode ? ` (${jwo.style.styleCode})` : ''}`;
        ...
        for (let i = 0; i < lots.length; i++) {
          ...
          const component = await tx.job_work_order_components.create({
            data: {
              ...
              declaredValue:
                cost != null ? new Prisma.Decimal(toNumber(roundToCent(multiplyCurrency(qty, cost)))) : null,
          ...
          const challanItem = challan.items.find((it) => it.greigeStockId === row.id);
          if (challanItem) {
      ...
      // 6. CHALLAN → ISSUED (dispatch is real; also disarms the challan-page re-consume)
