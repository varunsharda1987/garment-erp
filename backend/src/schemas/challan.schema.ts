...
const challanItemSchema = z.object({
  ...
  description: z.string().min(1, 'Description is required').max(500, 'Description must not exceed 500 characters').trim(),
...
export const createChallanSchema = z.object({
  ...
  lrNumber: z
    .string()
    .max(50, 'LR number must not exceed 50 characters')
    .trim()
    .optional(),
