...
13. Review **Trims Details** (use **Add Trim** to add a row), **Embroidery Details** and **Accessories Details** the same way. A trim's **Qty** and **Rate** are in its material's own unit, shown beside them — metres for lace, elastic, interlining (fusing) and drawstring, e.g. **Qty (m)** and **(per m)**; pieces for buttons, labels and zippers. The unit comes from the material master, so there is nothing to pick; changing a row's material type clears it until you choose the new material. Lace is not in a trim row's **Type** list — cost lace in **Lace Details** (step 10).
