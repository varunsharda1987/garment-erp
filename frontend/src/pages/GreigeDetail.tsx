  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center h-64">
        <div className="text-lg text-muted-foreground">Loading greige details...</div>
      </div>
    );
  }
  ...
  if (error || !greige) {
    return (
      <div className="flex flex-col items-center justify-center h-64">
        <div className="text-lg text-destructive mb-4">{error || 'Greige not found'}</div>
  ...
  return (
    <>
      ...
      <div className="mb-4 text-sm text-muted-foreground">
        ...
        {' > '}
        <span className="font-medium text-foreground">{greige.greigeCode}</span>
      </div>
          ...
          <CardContent>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              <div>
                ...
                <div className="text-base font-semibold text-foreground">{greige.greigeCode}</div>
              </div>
              <div>
                <label className="text-sm font-medium text-muted-foreground">Greige Name</label>
                <div className="text-base text-foreground">{greige.greigeName}</div>
              </div>
              <div>
                <label className="text-sm font-medium text-muted-foreground">Composition</label>
                <div className="text-base text-foreground">{greige.composition}</div>
              </div>
              {greige.yarnCount && (
                <div>
                  <label className="text-sm font-medium text-muted-foreground">Yarn Count</label>
                  <div className="text-base text-foreground">{greige.yarnCount}</div>
                </div>
              )}
              {greige.construction && (
                <div>
                  <label className="text-sm font-medium text-muted-foreground">Construction</label>
                  <div className="text-base text-foreground">{greige.construction}</div>
                </div>
              )}
              {greige.weaveType && (
                <div>
                  <label className="text-sm font-medium text-muted-foreground">Weave Type</label>
                  <div className="text-base text-foreground">{greige.weaveType}</div>
                </div>
              )}
              {greige.gsmRange && (
                <div>
                  <label className="text-sm font-medium text-muted-foreground">GSM Range</label>
                  <div className="text-base text-foreground">{greige.gsmRange}</div>
                </div>
              )}
            </div>
            ...
            {greige.description && (
              <div className="mt-6">
                ...
                <div className="text-base text-foreground mt-1">{greige.description}</div>
              </div>
            )}
          ...
          <CardContent>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
              <div>
                ...
                <div className="text-base font-semibold text-foreground">{Number(greige.greigeWidth)}"</div>
              </div>
              {greige.expectedFinishedWidthMin && (
                <div>
                  <label className="text-sm font-medium text-muted-foreground">Min Finished Width</label>
                  <div className="text-base text-foreground">{Number(greige.expectedFinishedWidthMin)}"</div>
                </div>
              )}
              {greige.expectedFinishedWidthMax && (
                <div>
                  <label className="text-sm font-medium text-muted-foreground">Max Finished Width</label>
                  <div className="text-base text-foreground">{Number(greige.expectedFinishedWidthMax)}"</div>
                </div>
              )}
              <div>
          ...
          <CardContent>
            {finishedFabrics.length > 0 ? (
                ...
                <table className="min-w-full divide-y divide-gray-200">
                  ...
                  <tbody className="bg-card divide-y divide-gray-200">
                    {finishedFabrics.map((fabric) => (
                      <tr key={fabric.id} className="hover:bg-muted">
                        <td className="px-4 py-3 text-sm font-medium text-info">
                          <Link to={`/fabric/${fabric.id}`} className="hover:text-info">
                            {fabric.fabricCode}
                          </Link>
                        </td>
                        <td className="px-4 py-3 text-sm text-foreground">{fabric.fabricName}</td>
                        <td className="px-4 py-3 text-sm text-foreground">{fabric.colorName || 'N/A'}</td>
                        <td className="px-4 py-3 text-sm text-foreground">{fabric.finishType || 'N/A'}</td>
                        <td className="px-4 py-3 text-sm text-foreground">{Number(fabric.actualWidth)}"</td>
              ...
              <div className="text-center py-8">
                ...
                <div className="text-sm text-muted-foreground">No finished fabrics created from this greige yet</div>
                <Link to={`/fabric/new?greigeId=${id}`}>
        ...
        <Card>
          ...
          <CardContent>
            {stockEntries.length > 0 ? (
                ...
                <table className="min-w-full divide-y divide-gray-200">
                  ...
                  <tbody className="bg-card divide-y divide-gray-200">
                    {stockEntries.map((stock) => (
                      <tr key={stock.greigeId} className="hover:bg-muted">
                        <td className="px-4 py-3 text-sm text-foreground font-medium">{stock.greigeCode}</td>
                        <td className="px-4 py-3 text-sm text-foreground">{stock.greigeName}</td>
                        <td className="px-4 py-3 text-sm text-foreground">{stock.composition}</td>
              ...
              <div className="text-center py-8">
                <Package className="h-12 w-12 text-muted-foreground mx-auto mb-3" />
                <div className="text-sm text-muted-foreground">No stock entries for this greige yet</div>
                <Link to={`/greige-stock-entry?greigeId=${id}`}>
                  <Button variant="outline" size="sm" className="mt-3">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              {greige.createdBy && (
                <div>
                  ...
                  <div className="text-sm text-muted-foreground">{greige.createdBy.email}</div>
                </div>
              )}
            </div>
          </CardContent>
