...
export const createEmbroidery = async (req: Request, res: Response) => {
  try {
    ...
    logError('Error creating embroidery:', error);
    res.status(500).json({ error: 'Failed to create embroidery design', details: error.message });
  }
};
...
export const getAllEmbroidery = async (req: Request, res: Response) => {
  try {
    ...
    logError('Error fetching embroidery designs:', error);
    res.status(500).json({ error: 'Failed to fetch embroidery designs', details: error.message });
  }
};
...
export const getEmbroideryById = async (req: Request, res: Response) => {
  try {
    ...
    if (!embroidery) {
      return res.status(404).json({ error: 'Embroidery design not found' });
    }
    ...
    logError('Error fetching embroidery design:', error);
    res.status(500).json({ error: 'Failed to fetch embroidery design', details: error.message });
  }
};
...
export const updateEmbroidery = async (req: Request, res: Response) => {
  try {
    ...
    logError('Error updating embroidery:', error);
    res.status(500).json({ error: 'Failed to update embroidery design', details: error.message });
  }
};
  ...
  try {
    ...
    if (existing._count.style_fabrics > 0) {
      return res.status(400).json({
        error: 'Cannot delete embroidery design',
        message: `This design is used in ${existing._count.style_fabrics} style fabric(s). Please remove from styles first or deactivate instead.`,
      });
    }
    ...
    logError('Error deleting embroidery:', error);
    res.status(500).json({ error: 'Failed to delete embroidery design', details: error.message });
  }
};
...
export const searchEmbroidery = async (req: Request, res: Response) => {
  try {
    ...
    logError('Error searching embroidery:', error);
    res.status(500).json({ error: 'Failed to search embroidery designs', details: error.message });
  }
};
