 ...
 *
 * dotenv never overwrites a key already set, so the first file wins (.env.local over .env), and a
 * variable PM2 does pass still takes priority over both. app.ts keeps its own identical calls; they
 * are no-ops once this has run.
 */
