...
const stockMovement_service_1 = __importDefault(require("./stockMovement.service"));
...
class StockCountService {
    ...
    async createStockCount(data) {
        return await prisma.$transaction(async (tx) => {
            ...
            // Create count items
            const countItems = await Promise.all(materialsToCount.map((stockLevel) => tx.stock_count_items.create({
                data: {
     ...
     */
    async getAllStockCounts(filters) {
        const where = {};
     ...
     */
    async updateCountItem(itemId, data) {
        ...
        if (data.physicalQuantity !== undefined) {
            variance = new library_1.Decimal(data.physicalQuantity.toString()).sub(item.systemQuantity.toString());
        }
     ...
     */
    async updateCountProgress(stockCountId) {
        const items = await prisma.stock_count_items.findMany({
            where: { stockCountId },
        });
        const countedItems = items.filter((i) => i.physicalQuantity !== null).length;
        const varianceItems = items.filter((i) => i.variance !== null && !new library_1.Decimal(i.variance.toString()).eq(0)).length;
     ...
     * Verify stock count
     */
    async verifyStockCount(id, verifiedById) {
        const stockCount = await prisma.stock_counts.findUnique({
            where: { id },
        });
        if (!stockCount) {
     ...
     */
    async approveStockCount(id, approvedById) {
        return await prisma.$transaction(async (tx) => {
            ...
            for (const item of stockCount.stock_count_items) {
                if (item.variance && !new library_1.Decimal(item.variance.toString()).eq(0)) {
                    const varianceQty = new library_1.Decimal(item.variance.toString());
                    // Create stock adjustment
                    const adjustment = await stockMovement_service_1.default.createStockAdjustment({
    ...
    async getVarianceReport(id) {
        ...
        const varianceItems = stockCount.stock_count_items.filter((item) => item.variance && !new library_1.Decimal(item.variance.toString()).eq(0));
        const totalVariance = varianceItems.reduce((sum, item) => {
            return sum + Math.abs(parseFloat(item.variance?.toString() || '0'));
        }, 0);
        const positiveVariance = varianceItems.filter((item) => new library_1.Decimal(item.variance?.toString() || '0').gt(0));
        const negativeVariance = varianceItems.filter((item) => new library_1.Decimal(item.variance?.toString() || '0').lt(0));
        return {
            stockCount,
            varianceItems,
     ...
     */
    async generateCountNumber(warehouseId) {
     ...
     */
    async getCountSummary(warehouseId, startDate, endDate) {
...
exports.default = new StockCountService();
