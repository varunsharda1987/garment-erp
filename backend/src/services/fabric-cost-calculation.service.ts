...
async function calculateGreigeProcessingCost(
  ...
  // Get greige cost (WAC from stock or greige master cost)
  const greigeStock = await prisma.fabric_stock.findFirst({
    where: {
      fabricId: fabric.greigeId,
      status: 'AVAILABLE',
      quantityAvailable: { gte: quantityNeeded },
    },
    orderBy: { weightedAvgCost: 'asc' },
  });

  const greigeCostPerMeter = greigeStock
    ? Number(greigeStock.weightedAvgCost)
