...
6. **Label Name**: leave it empty to have it built automatically from Type, Colour, Material and Size (the word "Label" is added only for sewn-in labels). Type a name only to override.
...
8. **Customer (Optional)** and **Brand (Optional)** link the label to one buyer. Leave **No Customer (Generic Label)** for a generic label. Choose the customer first — the **Brand (Optional)** dropdown only appears after that, and stays disabled with "No brands available" if that customer has no brands set up.
...
13. To add a supplier, go to the **Suppliers** section, click **Add Supplier** and pick the **Supplier** (the only required field in the row). Fill **Price/Piece (₹)**, **Price/100 (₹)** and **Notes** if known. Tick **Preferred Supplier** for the main source (the first row is ticked automatically) and keep **Active** ticked. Rows with no supplier chosen are dropped silently on save — remove them with the bin icon.
14. Optional: enter the **Supplier Reference Code** under **Reference Codes**, and any **Description** notes under **Additional Information**.
15. Click **Create Label** (or **Create Hangtag** / **Create Price Tag**). You return to the **Label Management** list. **Cancel** returns without saving.
