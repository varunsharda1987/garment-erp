...
export const createBankAccountSchema = z.object({
  accountNumber: z
    ...
    .trim(),
  bankName: z.string().min(1, 'Bank name is required').max(200, 'Bank name must not exceed 200 characters').trim(),
  branchName: z
    .string()
    .min(1, 'Branch name is required')
    .max(200, 'Branch name must not exceed 200 characters')
    .trim(),
  ifscCode: z.string().max(20, 'IFSC code must not exceed 20 characters').trim().optional().nullable(),
  swiftCode: z.string().max(20, 'SWIFT code must not exceed 20 characters').trim().optional().nullable(),
  ...
  accountHolderName: z
    ...
    .trim(),
  openingBalance: z.number().nonnegative('Opening balance must be non-negative').optional().default(0),
  currency: z.string().max(10, 'Currency must not exceed 10 characters').trim().optional().default('INR'),
...
export const bankAccountQuerySchema = z.object({
  ...
  sortBy: z.enum(['accountNumber', 'bankName', 'accountType', 'createdAt']).optional().default('createdAt'),
