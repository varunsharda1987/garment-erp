/**
 * Embroidery Piece Receive Page
 * Record receipt of embroidered cut pieces from vendor
 */
...
export default function EmbroideryPieceReceive() {
  ...
  const { data: pendingSendOuts } = useQuery({
    queryKey: ['emb-piece-pending'],
    queryFn: () => externalProcessService.getSendOuts({ processType: 'EMBROIDERY_PIECE', status: 'SENT', limit: 100 }),
    enabled: !id,
  });

  const { data: sendOut } = useQuery({
    queryKey: ['emb-piece-sendout', selectedSendOutId],
  ...
  return (
    <div className="space-y-6 p-6">
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-2xl font-display font-medium">Embroidery Piece Receive</h1>
          <p className="text-muted-foreground">Record receipt of embroidered pieces from vendor</p>
        </div>
      </div>
          ...
          <CardContent>
            <div className="grid grid-cols-2 gap-3 md:grid-cols-4 text-sm">
              ...
              <div>
                ...
                <strong>{sendOut.embroidery?.designName || '—'}</strong>
              </div>
              <div>
                  ...
                  return (
                    <TableRow key={sku.id}>
                      ...
                      <TableCell className="text-right font-medium">
                        {Math.max(0, recv.received - recv.damaged)}
                      </TableCell>
                    </TableRow>
                  );
                })}
