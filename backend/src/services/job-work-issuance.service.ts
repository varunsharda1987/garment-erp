  ...
  const result = await prisma.$transaction(
    async (tx) => {
      ...
      return {
        ...
        drawnAt: processorLots.length > 0 ? (jwo.processor?.name ?? 'the processor') : null,
        coveringChallans: [
          ...new Set(processorLots.map((l) => l.row.sourceChallan?.challanNumber).filter((n): n is string => !!n)),
        ].join(', '),
        warnings,
      };
    },
