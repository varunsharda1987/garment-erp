      ...
      try {
        ...
        if (matchingCustomer) {
          ...
          if (matchingCustomer.brandCategories && matchingCustomer.brandCategories.length > 0) {
            const uniqueBrands = [
              ...new Set(matchingCustomer.brandCategories.map((bc: BrandCategory) => bc.brandName)),
            ];
        ...
        if (styleMaterialBomData.length > 0) {
          type BOMItem = {
            materialId: string;
            material?: { name?: string; code?: string; type?: string };
            estimatedConsumption?: number;
            unit?: string;
            notes?: string;
        ...
        if (style.components?.length > 0) {
          payload.selectedComponents = style.components.map(
            (sc: { componentName?: string; componentType?: string }) => {
  ...
  const saveStyle = useCallback(
    async (isDraft: boolean) => {
      ...
      try {
        ...
        const hasThread = [...materialBOM, ...accessories].some((m) => m.materialType === 'THREAD');
