...
export default function SmockingDashboard() {
  ...
  const { data: dashboard } = useQuery({
    queryKey: ['smocking-dashboard'],
    queryFn: () => externalProcessService.getDashboard('SMOCKING'),
  });
  ...
  const cancelMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({ queryKey: ['smocking-sendouts'] });
      queryClient.invalidateQueries({ queryKey: ['smocking-dashboard'] });
      setCancelId(null);
      setCancelReason('');
    },
    onError: (error: any) => {
  ...
  return (
    <div className="space-y-6 p-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-display font-medium">Smocking</h1>
          <p className="text-muted-foreground">Track smocking send-outs and WIP</p>
        </div>
        <Link to="/manufacturing/smocking/send-out">
          <Button>
        ...
        <div className="grid grid-cols-2 gap-4 md:grid-cols-5">
          <Card>
            <CardContent className="pt-4">
              ...
              <p className="text-xs text-muted-foreground">{summary.totalQtySent} qty</p>
            </CardContent>
          </Card>
          <Card>
            <CardContent className="pt-4">
              ...
              <p className="text-xs text-muted-foreground">{summary.totalQtyPending} qty pending</p>
            </CardContent>
          </Card>
          <Card>
            <CardContent className="pt-4">
          ...
          <Card>
            <CardContent className="pt-4">
              ...
              <p className="text-xs text-muted-foreground">{summary.totalQtyReceived} qty received</p>
            </CardContent>
          </Card>
          <Card>
            <CardContent className="pt-4">
                  ...
                  <TableRow key={s.id} className={isOverdue(s) ? 'bg-destructive/10' : ''}>
                    ...
                    <TableCell>
                      <div className="flex gap-1">
                        {(s.status === 'SENT' || s.status === 'PARTIALLY_RECEIVED') && (
                          <Link to={`/manufacturing/smocking/receive/${s.id}`}>
