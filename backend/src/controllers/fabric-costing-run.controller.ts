...
export async function getRunsByStyle(req: Request, res: Response) {
  try {
    ...
    console.error('Error getting costing runs:', error);
    res.status(500).json({
      success: false,
      error: error.message || 'Failed to get costing runs',
    });
  }
}
...
export async function getRunById(req: Request, res: Response) {
  try {
    ...
    console.error('Error getting costing run:', error);
    res.status(500).json({
      success: false,
      error: error.message || 'Failed to get costing run',
    });
  }
}
...
export async function createRun(req: Request, res: Response) {
  try {
    ...
    if (!fabricCadIds || !Array.isArray(fabricCadIds) || fabricCadIds.length === 0) {
      return res.status(400).json({
        success: false,
        error: 'At least one fabric CAD ID is required',
      });
    }
    ...
    console.error('Error creating costing run:', error);
    res.status(500).json({
      success: false,
      error: error.message || 'Failed to create costing run',
    });
  }
}
...
export async function deleteRun(req: Request, res: Response) {
  try {
    ...
    console.error('Error deleting costing run:', error);
    res.status(500).json({
      success: false,
      error: error.message || 'Failed to delete costing run',
    });
  }
}
...
export async function updateRunTotals(req: Request, res: Response) {
  try {
    ...
    console.error('Error updating costing run totals:', error);
    res.status(500).json({
      success: false,
      error: error.message || 'Failed to update costing run totals',
    });
  }
}
