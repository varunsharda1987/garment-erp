...
export const getSummary = async (req: Request, res: Response) => {
  try {
    ...
    console.error('Error fetching trim summary:', error);
    res.status(500).json({ error: 'Failed to fetch trim summary', details: error.message });
  }
};
...
export const searchAllTrims = async (req: Request, res: Response) => {
  try {
    ...
    if (typeFilter && activeConfigs.length === 0) {
      return res.status(400).json({ error: 'Invalid trim type', validTypes: trimConfigs.map(c => c.type) });
    }
    ...
    res.status(500).json({ error: 'Failed to search trims', details: error.message });
  }
};
...
export const getRecentTrims = async (req: Request, res: Response) => {
  try {
    ...
    console.error('Error fetching recent trims:', error);
    res.status(500).json({ error: 'Failed to fetch recent trims', details: error.message });
  }
};
