...
export const returnUnprocessedProcessPO = async (req: Request, res: Response, _next: NextFunction) => {
  ...
  // this page, the Dyeing page and the Job Work Order screen cannot drift into three different
  // versions of "the mill sent it back untouched". The guard that used to sit here read
  // `job.status` — the legacy JobWorkStatus column, retired and dropped — so it saw undefined,
  // never matched AT_MILL and refused every job; the service checks `jwoStatus` instead.
