...
- **Co-owned:** `greigeCostPerMeter` / `greigeRateSource*` — CAD Planning's auto-costing seeds them and the Fabric Costing save rewrites them. `costInputMode` is costing-owned; its ONLY legal values are `BUILD_UP` | `LANDED_PRICE`.
- **The greige rate has ONE live lookup and an honest label** (2026-09-26, `backend/src/services/helpers/greige-live-rate.helper.ts`). Live rate = the NEWEST placed greige PO line (SENT / ACKNOWLEDGED / PARTIALLY_RECEIVED / RECEIVED / SHORT_CLOSED, per metre) or receipt (`fabric_procurement`, priced `greige_stock` lot); on the same day a receipt beats the PO; else the greige master default. Fabric Costing, CAD auto-costing, Push to Fabric Costing and the cost-sheet calculator all call `resolveLiveGreigeRates` — never write a private copy (there were four, all blind to POs: PO2609-0004's ₹67 never reached costing). A saved rate's label comes from `greigeRateProvenance`: equal to the live rate → its source + document (`greigeRateSourceRef` = PO number); different → `MANUAL_OVERRIDE` with a REQUIRED reason; a re-sent untouched rate keeps its label. Sweep: `check-order-system-integrity` D19. A saved processing **rate card prices ONE greige**: the save refuses a `rateCardId` whose greige is not the row's, and the costing page re-looks-up the rate when a CAD greige change left the old card (`rateCardNeedsRelookup`). Sweep D20.
...
| Rate-card printing type | A `processor_rate_card` lookup whose `where` filters `processingType` but not `printingType`. PIGMENT / PROCIAN / DISCHARGE / PIGMENT_DISCHARGE are separate open-ended cards in ONE slab, so the lookup picks whichever sorts first: ESSKY082LS (Pigment ₹20) was compared against Procian ₹28 and refused its Order BOM as `RATES_OUTDATED` +40%, which a new cost-sheet version cannot clear (2026-09-26) | Put the card's own `printingType` in the where (`null` for dyeing); a literal `'DYEING'` query is skipped. A deliberate read across print types takes `// allow-any-print-type` |
