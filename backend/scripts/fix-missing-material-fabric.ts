/**
 * One-time fix: create the missing materials record for fabric FAB-EMFK00262-001
 * (was never created because the controller bypassed materialService.createFromMaster)
 *
 * Run: cd backend && npx ts-node scripts/fix-missing-material-fabric.ts
 */
...
async function fixMissingMaterial() {
  const FABRIC_CODE = 'FAB-EMFK00262-001';
