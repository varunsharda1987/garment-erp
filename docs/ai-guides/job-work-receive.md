...
When the processor sends the material back without working on it at all — wrong shade, they could not take the job — open the job work order and click **Returned unprocessed** in the Actions card. It sits under **Receive from processor** and shows only while the job is at **Issued**, **In Transit** or **At Processor** and nothing has been received yet. It is refused for a job that took cloth already lying at the processor (delivered straight there — no challan went out): nothing travelled, so nothing can come back to our store from it. Cancel that job and choose **At Processor** instead; the cloth goes back on the processor's stock for the next job there. The inward challan of a real return names the store the goods came back into.
