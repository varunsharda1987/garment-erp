...
| 12 | "That schema gap only affects direct API callers — the form validates the fields." | Checked `CuttingForm.tsx`; the runbook's path is `CuttingChart.tsx`, which posts zeros to the same route and had been refused since April 2026. The end-to-end walk missed it too — it posted values the page never sends. | Two callers, two payloads: probe each caller's exact request against the compiled schema (`node -e`). A walk must post what the PAGE posts. Struck again 2026-09-25 on the very next step: Save Lay's schema wanted a `sizeName` the batch page never sends (`0a9a13a9`) — the walk had stopped at batch creation. Walk to the END state, not to the first success. |
...
| 13 | "Every remaining item is latent — nothing blocks the first real run" (plan Step 6, 2026-09-17). | The push-to-cutting stock gate aggregated `fabric_stock` on `order_bom_items.fabricId`, which is null BY DESIGN at BOM time — so every ORDER-backed run read "Available 0.00" with 1,704 m in stock (found 2026-09-21 preparing the first real cut, fixed `108242f2`). The walk never saw it: it is a STOCK run, and stock runs skip the BOM check. | Trap #3 on the READ side: a gate that reads the design-time column while the rows live under the lineage columns (`fabric_master.greigeId`, `fabric_stock.originStyleId`). A walk proves only the path it takes — walk the order-backed variant too. |
